...
GenerationOption = {}
  ...
  if (staticPath) {
    const fileContent = generateFile(data.static, data.plugins, compatibility, 'static')

    saveFile(fileContent, staticPath)
  }

  if (dynamicPath) {
    const fileContent = generateFile(data.dynamic, data.plugins, compatibility, 'dynamic')
