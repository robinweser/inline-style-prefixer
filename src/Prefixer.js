      ...
      if (hackData.alternativeProperty) {
        ...
        if (oldProperty) {
          if (oldProperty instanceof Array) {
            oldProperty.forEach(prop => {
              let oldValue = hackData.alternativeValue[prop];
              let alternativeValue = oldValue && oldValue.hasOwnProperty(value) ? oldValue[value] : value
              styles[prop] = alternativeValue
            })
          } else {
            let oldValue = hackData.alternativeValue[oldProperty]
            let alternativeValue = oldValue && oldValue.hasOwnProperty(value) ? oldValue[value] : value
